...
*/
  class core {
	...
	{
		$this->rConfig = json_decode(file_get_contents($this->config_path), true);
    ...
    {
		$sUrl = $this->rConfig['sDsmuri'].'/?key='.$this->rConfig['sDsmkey'].'&query='.base64_encode($sQuery);
		$context = stream_context_create(array('http'=>array('method'=>"GET")));
		$jsonData = json_decode(file_get_contents($sUrl, false, $context), true);
		return  ($jsonData);
    }
    ...
    public function controlerBridge() {
      ...
      $arrControlerReference = array(
        'main' => 'mainControler',
        'auth' => 'authentificationControler',
        'install' => 'installControler',
        'panel' => 'panelControler'
      );
